...
class CreateRdsRfPubTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('rds_rf_pub', function(Blueprint $table){
            $table->increments('id');
            $table->string('STATUS')->nullable()->comment('Статус');
            $table->string('DECL_NUM')->unique()->comment('Номер декларации');

            $table->text('a_cert_type-cert_rf')->nullable()->comment('Выбор раздела');
...
//            $table->text('')->nullable()->comment('Сведения об испытательной лаборатории. Прочие документы, послужившие основанием выдачи сертификата');
            ...
            $table->text('a_product_info-rds-product_rf-object_type_cert')->nullable()->comment('Сведения о продукции. «Тип объекта декларирования»: серийный выпуск, партия, единичное изделие');
            $table->text('a_product_info-rds-product_rf-product_type')->nullable()->comment('Сведения о продукции. Происхождение продукции');
            $table->text('a_product_info-rds-product_rf-product_name')->nullable()->comment('Сведения о продукции. Полное наименование продукции');
            $table->text('a_product_info-rds-product_rf-product_info')->nullable()->comment('Сведения о продукции. Сведения о продукции (тип, марка, модель, сорт, артикул и др.), обеспечивающие ее идентификацию');
            $table->text('a_product_info-rds-product_rf-okpd2')->nullable()->comment('Сведения о продукции. Коды ОКПД 2');
            $table->text('a_product_info-rds-product_rf-okpd2_text')->nullable()->comment('Сведения о продукции. Коды ОКПД 2');
            $table->text('a_product_info-rds-product_rf-tn_ved')->nullable()->comment('Сведения о продукции. Код ТН ВЭД ЕАЭС');
            $table->text('a_product_info-rds-product_rf-tn_ved_text')->nullable()->comment('Сведения о продукции. Код ТН ВЭД ЕАЭС');
            $table->text('a_product_info-rds-product_rf-name_doc_made_product')->nullable()->comment('Сведения о продукции. Наименование и реквизиты документа, в соответствии с которыми изготовлена продукция');
            $table->text('a_product_info-rds-product_rf-product_info_ext')->nullable()->comment('Сведения о продукции. Иная информация, идентифицирующая продукцию');
            $table->text('a_product_info-rds-product_rf-serial_number_product')->nullable()->comment('Сведения о продукции. Размер партии или заводской номер изделия');
            $table->text('a_product_info-rds-product_rf-requisites_doc')->nullable()->comment('Сведения о продукции. Реквизиты товаросопроводительной документации');
